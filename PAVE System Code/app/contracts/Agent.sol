...
contract Agent {
    ...
    function get_insurer(address addr) view public returns (string memory , uint){
        
        return (insurerInfo[addr].name, insurerInfo[addr].age);
    }
    ...
    function get_patient_insurer_name(address paddr, address daddr) view public returns (string memory , string memory ){
        return (patientInfo[paddr].name,insurerInfo[daddr].name);
    }
